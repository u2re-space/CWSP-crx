...
const processSelectedText = async () => {
    ...
    if (!text) {
        chrome.notifications.create({ type: "basic", iconUrl: "icons/icon.png", title: "CWSP-crx", message: "Select text first!" });
        return;
    }
    ...
    try {
        ...
        chrome.notifications.create({ type: "basic", iconUrl: "icons/icon.png", title: "CWSP-crx", message: result?.ok !== false ? "Text processed!" : `Failed: ${result?.error || "Unknown"}` });
    } catch (e) {
        chrome.notifications.create({ type: "basic", iconUrl: "icons/icon.png", title: "CWSP-crx", message: `Failed: ${e instanceof Error ? e.message : String(e)}` });
    }
};
