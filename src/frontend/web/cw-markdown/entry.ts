/*
 * Filename: entry.ts
 * FullPath: apps/CWSP-crx/src/frontend/web/cw-markdown/entry.ts
 * Change date and time: 22.20.00_19.07.2026
 * Reason for changes: VDS md.u2re.space / /markdown/ SPA — viewer + workcenter shell.
 */

/**
 * CWSP-crx Markdown host entry (Fastify apps/cw-markdown).
    ...
    void import("../../../index.ts")
        .then(async (mod) => {
            ...
            if (typeof run !== "function") {
                throw new Error("CWSP-crx default export is not a boot function");
            }
            await run(mount);
        })
        .catch((error: unknown) => {
            ...
            mount.textContent =
                error instanceof Error ? error.message : "Failed to start CWSP-crx Markdown";
        });
}
